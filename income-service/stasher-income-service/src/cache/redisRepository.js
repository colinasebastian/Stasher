...
module.exports = class RedisRepository {
 constructor(){
    ...
    async updateIncomes(category,key){
    ...
    async getIncomesByCategory(category,key){ 
       var keyredis = category+key;
      return JSON.parse(await this.client.get(keyredis)) 
    }

    async saveIncomesByCategory(category,key,incomes){
       var keyredis = category+key;
      await this.client.set(keyredis,JSON.stringify(incomes));
    }
}