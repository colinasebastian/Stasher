    ...
    components: {
        ...
        methods: {
            ...
            photoFormData.append("limitPerMonthExpense", e.target.limit.value);
            e.preventDefault();
            currentObj.errors = [];
            if (this.checkForm(e)){
                this.axios
                .put(this.url + "/categories/" + e.target.id.value, photoFormData, {
                headers: { 'authentication': JSON.parse(localStorage.getItem('user'))?.token,
                        'Content-Type': 'multipart/form-data' }
            })
                .then(function (response) {
                    currentObj.message = "Category modify successfully";
    ...
    <template>
    <section>
    <div class="flex-wrap w-3/4 mx-auto py-2 my-5">
        <form class="flex flex-col w-2/3 gap-1 mx-auto mb-2" id="myForm" @submit="formSubmit">
        <h1
        class=" mt-2 mx-auto text-center font-bold text-xl text-text text-slate-50 my-2 border-b-2 border-fg"
        >
        Modify Category
        </h1>
        <FormInput label="Id" :readonly="true" placeholder="Id of category..." name="id" v-model="id" :value="categoryId" type="Number" error="Incorrect id"></FormInput>
        <FormInput label="Name" placeholder="Name of category..." name="name" v-model="name" :value="categoryName" type="Text" error="Incorrect name"></FormInput>
        <FormInput label="Description" placeholder="Description of category..." name="description" v-model="description" :value="categoryDescription" type="Text" error="Incorrect description"></FormInput>     
        <FormInput label="Limit" placeholder="Limit of category..." name="limit" v-model="limit" :value="categoryAmount" type="Number" error="Incorrect limit"></FormInput>
        <div class="mt-4">
        ...
        <input label="send" name="send" value="Modify Category" type="submit" class=" bg-stone-200 hover:bg-stone-400 hover:text-stone-200 text-rose-900 font-bold py-2 px-4 inline-block mt-6 rounded w-full"
        />
